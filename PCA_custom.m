...
% repmat(mu, m ,1);
if version == 0 % matches what we learned in class, variance normalization in preprocessing 
    [m,n]=size(data);
    mu=nanmean(data);
...
elseif version == 1 % claims to capture variance more successfully, probably artificial
    [m,n]=size(data);
    mu2 = zeros(1,n);
    sigma = ones(1,n);
