...
class ImagingDataset(Dataset):
    def __init__(self, data_folder, label_folder, train_ids, val_ids, test_ids, mode='train', transform=None):
        # buraları düzeltmek gerek
        # zor degil: train, val, test idleri üzerinden loopla
        # train-test indislerin kaymaması mühim, aman hocam..
    ...
    def __getitem__(self, index):
        ...
        label = self.labels_dict[int(key)][int(cell_id)]
        if self.transform:
            image = self.transform(image)
        else:
            image = transforms.ToTensor(image)
        # TODO: labels
        # I may need to get the actual cell ind instead of this index, depends on how I save labels on Matlab
        # I think I saved the labels the correct way, so I should be fine
        # toc = time.time() - tic
        # print(toc)
        return image, label
